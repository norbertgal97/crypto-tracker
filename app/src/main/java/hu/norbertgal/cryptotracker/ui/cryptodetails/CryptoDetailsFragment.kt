...
class CryptoDetailsFragment(val id: Long) : Fragment(), CryptoDetailsScreen {
    ...
    override fun showCryptoDetails(crypto: Crypto?) {
        ...
        tvPrice.text = "$".plus(String.format("%.3f", crypto?.quote?.get("USD")?.price))
        ...
        tvPercentchange1h.text = String.format("%.3f", crypto?.quote?.get("USD")?.percentChange1h).plus("%")
        tvPercentchange24h.text = String.format("%.3f", crypto?.quote?.get("USD")?.percentChange24h).plus("%")
        tvPercentchange7d.text = String.format("%.3f", crypto?.quote?.get("USD")?.percentChange7d).plus("%")
        tvPercentchange30d.text = String.format("%.3f", crypto?.quote?.get("USD")?.percentChange30d).plus("%")
    }
